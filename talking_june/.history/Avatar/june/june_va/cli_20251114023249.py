...
async def consumer(text_queue: asyncio.Queue[str], tts_model: Optional[TTS], detected_lang_holder: dict):
    ...
    # ⚡ MAIN CONSUMER LOOP: Process chunks in parallel!
    with AudioIO() as audio_io:
        while not shutdown_event.is_set():
            try:
                ...
                # ⚡ OPTIMIZATION: Check if all processing is done
                # No items in queue - cleanup finished tasks
                active_tasks = {t for t in active_tasks if not t.done()}
                
                # If no active tasks, we're ready for next input
                if not active_tasks:
                    if current_app_state.get_value() != AppState.READY_FOR_INPUT:
                        current_app_state.set_value(AppState.READY_FOR_INPUT)
                        logger.info("✅ All audio processing complete - Ready for next input!")
                    # Longer wait when truly idle
                    await asyncio.sleep(0.15)
                else:
                    # Still processing - check more frequently
                    await asyncio.sleep(0.02)  # 20ms - very responsive!
