        ...
        if tts_model:
            try:
                try:
                    ...
                    logger.debug("Could not stop/unload pygame mixer cleanly")

                # Wait up to 2 seconds for playback to fully stop
                wait_start = time.time()
                while pygame.mixer.music.get_busy() and (time.time() - wait_start) < 2.0:
...
async def consumer(text_queue: asyncio.Queue[str], tts_model: Optional[TTS], detected_lang_holder: dict):
    ...
    def clean_text_for_tts(text: str) -> str:
        ...
        # Enhanced text processing with optimal language selection
        ...
        logger.debug(f"Text enhanced: '{text[:50]}...' → '{enhanced_text[:50]}...' (lang: {optimal_lang})")
    ...
    def process_text_for_tts(text: str) -> str:
        ...
        if detect_language_mode(text) == 'hinglish':
            text = polish_hinglish(text)
    ...
    with AudioIO() as audio_io:
        while not shutdown_event.is_set():
            try:
                ...
                clean_text = clean_text_for_tts(text_buffer)

                if tts_model:
                    try:
                        # Ensure any previous playback is stopped so the TTS writer can open the output file.
                        try:
                            fp = getattr(tts_model, "file_path", "out.wav")
                            if os.path.exists(fp) and pygame.mixer.music.get_busy():
                                pygame.mixer.music.stop()
                                # try to unload the music to release file handles on Windows
                                try:
                                    pygame.mixer.music.unload()
                                except Exception:
                                    # older pygame may not have unload()
                                    pass
                                # small wait for system to release file handle
                                time.sleep(0.05)
                        except Exception:
                            logger.debug("Could not stop/unload previous playback before generating new TTS")

                        # Pass detected language to TTS for matching voice
                        detected_lang = detected_lang_holder.get('language')
                        synthesis = tts_model.forward(clean_text, language_code=detected_lang)
                ...
                if synthesis:
                    ...
                    # If the TTS forward returned a file path (e.g., Google TTS), play it directly
