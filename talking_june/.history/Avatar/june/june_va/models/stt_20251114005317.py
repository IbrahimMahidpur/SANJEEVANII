        ...
        if hasattr(self, "_gc_client"):
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"✅ Google STT API call successful")
                    logger.info(f"   Response has {len(response_primary.results) if response_primary.results else 0} results")
                except Exception as api_error:
                    logger.error(f"❌ Google STT API call failed: {api_error}")
                    logger.error(f"   Config: lang=en-IN, sample_rate={sample_rate}, model=latest_long")
                    return ("", "en-IN")
                ...
                if response_primary.results and response_primary.results[0].alternatives:
                    ...
                    logger.info(f"  ✅ en-IN transcript: '{transcript_primary}'")
                    logger.info(f"     Confidence: {confidence_primary:.2f}")
                    logger.info(f"     Length: {len(transcript_primary)} chars, {len(transcript_primary.split())} words")
                    ...
                    # No results from API
                    logger.warning(f"❌ No transcription results from Google API")
                    logger.warning(f"   Response object: {response_primary}")
                    logger.warning(f"   Has results: {bool(response_primary.results)}")
                    if response_primary.results:
                        logger.warning(f"   First result has alternatives: {bool(response_primary.results[0].alternatives)}")
                        if not response_primary.results[0].alternatives:
                            logger.warning(f"   Result exists but has no alternatives - audio may be silence/noise")
                    else:
                        logger.warning(f"   Response has no results - audio not processable by Google")
                
                
                if not best_transcript:
                    logger.warning("⚠️ Empty transcript after processing")
                    logger.warning(f"   Audio was: {audio_duration:.2f}s, max_level={audio_max:.3f}, rms={audio_rms:.3f}")
                    logger.warning(f"   This means Google API didn't recognize any speech in the audio")
                    logger.warning(f"   Possible causes: background noise, unclear speech, or audio too quiet")
                    return ("", "en-IN")
