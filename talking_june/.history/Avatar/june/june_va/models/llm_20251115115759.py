...
try:
    # ConnectionError is raised by the underlying ollama client when the daemon is unreachable
...
class LLM(BaseModel):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        # Initialize Ollama client. If the Ollama daemon is not running or unreachable,
        # create a placeholder and mark availability as False so caller code can handle it.
        try:
            self.model = Client()
            self.ollama_available = True
        except Exception as e:
            # If the specific Ollama ConnectionError class is available, check for it
            is_conn_err = False
            ...
            if is_conn_err:
                logger.error("Failed to connect to Ollama daemon. Please ensure Ollama is installed and running. See https://ollama.com/download")
            else:
                logger.exception("Unexpected error initializing Ollama client")
    ...
    def exists(self) -> bool:
        ...
        """
        # If the Ollama client failed to initialize, report model as not existing
        if not getattr(self, "ollama_available", False) or self.model is None:
            return False
        ...
        try:
            ...
            # Provide clearer guidance if connection failed at runtime
            if OllamaConnectionError is not None and isinstance(e, OllamaConnectionError):
                import logging
                logger = logging.getLogger(__name__)
                logger.error("Failed to connect to Ollama daemon while checking model. Ensure Ollama is running and reachable at http://localhost:11434. See https://ollama.com/download")
                return False
            ...
            # Unknown exception - re-raise so callers can see it
            raise
...
- Show personality and warmth"""
        ...
        if lang_mode == 'hinglish':
            system_prompt = (
                ...
                "⚠️ PRONUNCIATION GUIDE - Common Hindi words:\n"
                "- 'hai' (है) = pronounced as 'hai' (NOT 'hoon')\n"
                "- 'hoon' (हूं) = first person 'am' (Main hoon)\n"
                "- 'ho' (हो) = second person 'are' (Tum ho)\n"
                "- 'hain' (हैं) = plural 'are' (Woh log hain)\n"
                "Use these correctly: 'Fever hai' (not 'fever hoon'), 'Main hoon' (not 'main hai').\n\n"
