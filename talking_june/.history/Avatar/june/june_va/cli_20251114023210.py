    ...
    with AudioIO() as audio_io:
        while not shutdown_event.is_set():
            try:
                ...
                # Cleanup finished tasks while waiting
                active_tasks = {t for t in active_tasks if not t.done()}
                
                # ⚡ FIX: Set READY_FOR_INPUT when queue is empty and all tasks done
                if not active_tasks and current_app_state.get_value() != AppState.READY_FOR_INPUT:
                    current_app_state.set_value(AppState.READY_FOR_INPUT)
                    logger.debug("✅ Ready for next input (queue empty, all tasks done)")
