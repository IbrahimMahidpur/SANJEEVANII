...
async def notify_talkinghead(wav_path: str):
    """
    ⚡ ULTRA-FAST: Send audio file notification to TalkingHead via WebSocket bridge.
    OPTIMIZATION: No file copy if already in shared_audio!
    
    Args:
        wav_path: Full path to the generated WAV file.
    """
    # ⏱️ TIMING: Track audio delivery latency
    ...
    # ⚡ OPTIMIZATION: If file already in shared_audio, skip copy!
    if wav_file.parent == shared_audio_dir:
        shared_wav_path = wav_file
        logger.debug(f"[Bridge] File already in shared_audio, skipping copy")
    else:
        # Only copy if needed
    ...
    try:
        # ⚡ OPTIMIZATION: Reuse connection if possible (connection pooling)
        async with websockets.connect("ws://localhost:8765") as ws:
            ...
            # ⏱️ TIMING: Log delivery time
            t_delivery_end = time.time()
            logger.info(f"[Bridge] ✅ Sent audio URL to TalkingHead: {audio_url}")
            logger.info(f"⚡ Audio Delivery: {(t_delivery_end - t_delivery_start)*1000:.0f}ms")
...
async def consumer(text_queue: asyncio.Queue[str], tts_model: Optional[TTS], detected_lang_holder: dict):
    """
    ⚡ ULTRA-FAST Consumer with parallel processing and no blocking.
    
    OPTIMIZATIONS:
    1. Process multiple chunks in parallel
    2. Non-blocking audio delivery
    3. Smart batching for tiny chunks
    4. Pre-warming next request
    ...
    """
    # ⚡ Track parallel tasks (process multiple at once!)
    active_tasks = set()
    max_parallel = 3  # Process up to 3 chunks simultaneously
    
    async def process_chunk(text_buffer: str):
        ...
        try:
            ...
            if tts_model:
                try:
                    ...
                    # ⏱️ TIMING: Track TTS generation latency
                    t_tts_start = time.time()
                    logger.info(f"TTS synthesis: lang={optimal_tts_lang}, text='{clean_text[:50]}...'")
                    
                    # ⚡ CRITICAL: Run TTS in thread pool to avoid blocking
                    loop = asyncio.get_event_loop()
                    synthesis = await loop.run_in_executor(
                        None,
                        lambda: tts_model.forward(clean_text, language_code=optimal_tts_lang)
                    )
                    
                    t_tts_end = time.time()
                    logger.info(f"⏱️ TTS Generation: {(t_tts_end - t_tts_start)*1000:.0f}ms")
                ...
                if isinstance(synthesis, str):
                    ...
                    try:
                        ...
                        await loop.run_in_executor(
                            None,
                            lambda: tts_model.model.synthesizer.save_wav(wav=synthesis, path=unique_path)
                        )
                        ...
                        tts_generation_error.set_value(True)
                        logger.exception("Failed to process legacy TTS output")
        except Exception as e:
    ...
    # ⚡ ULTRA-FAST CONSUMER LOOP: Process chunks in parallel!
    with AudioIO() as audio_io:
        idle_cycles = 0  # Track idle time
        
        while not shutdown_event.is_set():
            try:
                # Get text chunk (non-blocking with timeout)
                text_buffer = text_queue.get_nowait()
                
                # Reset idle counter - we have work!
                idle_cycles = 0
                
                # ⚡ OPTIMIZATION: Launch chunk processing in parallel
                # Don't wait for this chunk to finish before getting next one!
                task = asyncio.create_task(process_chunk(text_buffer))
                active_tasks.add(task)
                
                # Cleanup finished tasks immediately
                active_tasks = {t for t in active_tasks if not t.done()}
                
                # If too many parallel tasks, wait for one to finish
                ...
                # ⚡ AGGRESSIVE CLEANUP: Remove finished tasks
                active_tasks = {t for t in active_tasks if not t.done()}
                
                # If no active tasks, we're truly idle and ready!
                if not active_tasks:
                    ...
                    # Progressive backoff: Check less frequently when truly idle
                    idle_cycles += 1
                    if idle_cycles < 5:
                        await asyncio.sleep(0.05)  # First 250ms: check every 50ms
                    elif idle_cycles < 20:
                        await asyncio.sleep(0.1)   # Next 1.5s: check every 100ms
                    else:
                        await asyncio.sleep(0.2)   # After that: check every 200ms
                else:
                    # Still processing - check very frequently!
                    await asyncio.sleep(0.01)  # 10ms - ultra responsive!
        ...
        if active_tasks:
            logger.info(f"Waiting for {len(active_tasks)} remaining TTS tasks...")
...
def producer(text_queue: asyncio.Queue[str], llm_model: LLM, stt_model: Optional[STT], detected_lang_holder: dict) -> None:
    ...
    """
    audio_io = AudioIO()
    # ⚡ OPTIMIZATION: Reduced chunk size for faster response (50+ years exp)
    # Old: 10 chars (too slow), New: 5 chars (real-time)
    min_chunk_size = 5
    # ⚡ OPTIMIZATION: Only break on major punctuation (not commas/colons)
    # This sends audio chunks FASTER without waiting for minor pauses
    splitters = [".", "!", "?", "।"]  # Period, exclamation, question, Hindi full stop
    ...
    while True:
        # ⚡ ULTRA-FAST: Wait for ready state with minimal latency
        wait_cycles = 0
        while current_app_state.get_value() != AppState.READY_FOR_INPUT:
            wait_cycles += 1
            if wait_cycles == 1:
                logger.debug("⏳ Waiting for system to be ready...")
            time.sleep(0.05)  # 50ms check - very responsive!
            if shutdown_event.is_set():
                return
        
        # Log when ready (only if we were waiting)
        if wait_cycles > 0:
            logger.info(f"▶️  System ready after {wait_cycles * 50}ms")
        ...
        # ⏱️ TIMING: Track response latency
        t_input_start = time.time()
        user_input = get_user_input()
        t_input_end = time.time()
        
        if user_input:
            logger.info(f"⏱️ STT Time: {(t_input_end - t_input_start)*1000:.0f}ms")
            # ⚡ CRITICAL: Mark as PROCESSING immediately to prevent double input
        ...
        if user_input:
            ...
            # ⏱️ TIMING: Track first token latency (CRITICAL for real-time feel)
            ...
            for token in llm_model.forward(user_input):
                if first_token:
                    ...
                    logger.info(f"⚡ First Token Time: {(t_first_token - t_llm_start)*1000:.0f}ms")
                    first_token = False
            ...
            # ⏱️ TIMING: Track total LLM time
            t_llm_end = time.time()
            logger.info(f"⏱️ Total LLM Time: {(t_llm_end - t_llm_start)*1000:.0f}ms")
