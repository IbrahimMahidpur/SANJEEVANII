...
def run_multiple_ocr_passes(image_data):
    ...
    try:
        ...
        results = []
        # Enhanced Tesseract configuration for better handwriting recognition
        # --oem 1: Use LSTM OCR Engine (better for handwriting)
        # --psm 6: Assume a single uniform block of text
        # -c tessedit_char_whitelist: Allow only specific characters (optional)
        tesseract_config = '--oem 1 --psm 6 -c preserve_interword_spaces=1'
        def ocr_with_preprocessing(img, conf=0.8):
            if img is None:
                return None
            # 1. Grayscale
        ...
        if 'enhanced' in image_data and image_data['enhanced']:
            print("[DEBUG] Running Tesseract OCR on enhanced image (advanced preprocessing)...")
            img = cv2.imread(image_data['enhanced'])
            result = ocr_with_preprocessing(img, conf=0.92)
            if result:
                results.append(result)

        # Original image pass
        if 'original' in image_data and image_data['original']:
            print("[DEBUG] Running Tesseract OCR on original image (advanced preprocessing)...")
            img = cv2.imread(image_data['original'])
            result = ocr_with_preprocessing(img, conf=0.88)
            if result:
                results.append(result)
