...
def run_multiple_ocr_passes(image_data):
    ...
    try:
        ...
        def ocr_with_preprocessing(img, conf=0.8, config_idx=0, preprocessing_type='advanced'):
            if img is None:
                return None
            ...
            if preprocessing_type == 'advanced':
            ...
            elif preprocessing_type == 'simple':
                ...
                thresh = cv2.adaptiveThreshold(sharpened, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 25, 5)
            else:
                thresh = gray
            ...
            if text.strip():
                print(f\"[DEBUG] OCR extracted {len(text.strip())} characters with preprocessing={preprocessing_type}, psm={config_idx}\")
                return [[(None, [text.strip(), conf])]]
            return None

        # Pass 1: Enhanced image with advanced preprocessing
        if 'enhanced' in image_data and image_data['enhanced']:
            print(\"[DEBUG] Pass 1: Enhanced image with PSM 6 (advanced preprocessing)...\")
            img = cv2.imread(image_data['enhanced'])
            result = ocr_with_preprocessing(img, conf=0.95, config_idx=0, preprocessing_type='advanced')
            if result:
                results.append(result)

        # Pass 2: Enhanced image with PSM 3
        if 'enhanced' in image_data and image_data['enhanced']:
            print(\"[DEBUG] Pass 2: Enhanced image with PSM 3 (auto page)...\")
            img = cv2.imread(image_data['enhanced'])
            result = ocr_with_preprocessing(img, conf=0.93, config_idx=1, preprocessing_type='advanced')
            if result:
                results.append(result)

        # Pass 3: Original image with simple preprocessing
        if 'original' in image_data and image_data['original']:
            print("[DEBUG] Pass 3: Original image with simple preprocessing...")
            img = cv2.imread(image_data['original'])
            result = ocr_with_preprocessing(img, conf=0.90, config_idx=0, preprocessing_type='simple')
            if result:
                results.append(result)

        # Pass 4: Original image with aggressive preprocessing (for poor quality)
        if 'original' in image_data and image_data['original']:
            print("[DEBUG] Pass 4: Original image with aggressive preprocessing...")
            img = cv2.imread(image_data['original'])
            result = ocr_with_preprocessing(img, conf=0.88, config_idx=1, preprocessing_type='aggressive')
            if result:
                results.append(result)

        # Pass 5: Enhanced image with PSM 11 (sparse text)
        if 'enhanced' in image_data and image_data['enhanced']:
            print("[DEBUG] Pass 5: Enhanced image with PSM 11 (sparse text)...")
            img = cv2.imread(image_data['enhanced'])
            result = ocr_with_preprocessing(img, conf=0.85, config_idx=2, preprocessing_type='advanced')
            if result:
                results.append(result)

        # Pass 6: Original with PSM 12
        if 'original' in image_data and image_data['original']:
            print("[DEBUG] Pass 6: Original image with PSM 12...")
            img = cv2.imread(image_data['original'])
            result = ocr_with_preprocessing(img, conf=0.83, config_idx=3, preprocessing_type='simple')
            if result:
                results.append(result)

        # Pass 7: Inverted image (for negative images)
        if 'enhanced' in image_data and image_data['enhanced']:
            print("[DEBUG] Pass 7: Inverted image...")
            enhanced_img = cv2.imread(image_data['enhanced'])
            if enhanced_img is not None:
                inverted = cv2.bitwise_not(enhanced_img)
                result = ocr_with_preprocessing(inverted, conf=0.80, config_idx=0, preprocessing_type='simple')
                if result:
                    results.append(result)
...
def extract_medical_entities(text):
    ...
    # Extract words and n-grams for better matching
    text_words = re.findall(r'\\b[a-z]{3,}\\b', text_lower)
    
    # Create bigrams and trigrams for multi-word medication matching
    ...
    all_ngrams = text_words + bigrams + trigrams
    
    print(f\"[DEBUG] Extracted {len(text_words)} words, {len(bigrams)} bigrams, {len(trigrams)} trigrams for medication matching\")
    ...
    for key in MEDICATION_DICT.keys():
        ...
        if key in text_lower:
            if key not in medications:
                ...
                print(f\"[MATCH] Exact match found: {key}\")
            continue
        ...
        for alias in MEDICATION_DICT[key]:
            if alias in text_lower:
                if key not in medications:
                    medications.append(key)
                    print(f\"[MATCH] Alias exact match found: {alias} -> {key}\")
                alias_found = True
                break
        ...
        # Try fuzzy matching with all n-grams
        best_match_score = 0
        best_match_ngram = None
        
        for ngram in all_ngrams:
            if len(ngram) < 4:
                continue
            
            # Check medication name fuzzy match (65% threshold - reduced for better recall)
            ratio_score = fuzz.ratio(ngram, key)
            partial_score = fuzz.partial_ratio(ngram, key)
            token_score = fuzz.token_sort_ratio(ngram, key)
            ...
            for alias in MEDICATION_DICT[key]:
                ...
                ratio_score = fuzz.ratio(ngram, alias)
                partial_score = fuzz.partial_ratio(ngram, alias)
                token_score = fuzz.token_sort_ratio(ngram, alias)
        ...
        # If we found a good match (65% threshold), add it
        if best_match_score >= 65 and key not in medications:
            medications.append(key)
            print(f\"[MATCH] Fuzzy match found: '{best_match_ngram}' -> {key} (score: {best_match_score})\")
